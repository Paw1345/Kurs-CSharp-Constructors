...
namespace CpuMonitor_GUI
{
    public partial class Form1 : Form
    {
        ...
        {
            mesg = "Witaj Przyjacielu!!";
            mesg_text.Invoke(new MethodInvoker(setmesg));
            aSpeak(mesg);

            mesg = "To skaner użycia CPU i Ram dla Ciebie";
            mesg_text.Invoke(new MethodInvoker(setmesg));
            aSpeak(mesg);

            mesg = "Zwróc uwagę na praktycznośc i pamietaj zwrócic błędy";
            mesg_text.Invoke(new MethodInvoker(setmesg));
            aSpeak(mesg);
            ...
            String cpuMesg = String.Format("W tym momencie użycie procesora to {0}%", cval);
            String ramMesg = String.Format("Teraz używasz {0}% ramu", rval);
            ...
            while (true)
            {
                ...
                if (cflag.Sum() == 10)
                {
                    ...
                    if (ts.Seconds > 10)
                    {
                        mesg = "ALERT!! Zbyt wysokie złożycie";
                        mesg_text.Invoke(new MethodInvoker(setmesg));
                        aSpeak(mesg);

                        mesg = "System zostanie zrestartowany";
                        mesg_text.Invoke(new MethodInvoker(setmesg));
                        aSpeak(mesg);

                        mesg = "Zapisz swoja pracę";
                        mesg_text.Invoke(new MethodInvoker(setmesg));
                        aSpeak(mesg);
